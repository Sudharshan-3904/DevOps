...
export default function App() {
    ...
    if (currentLocation && currentLocation.toUpperCase() !== "USA") {
    ...
    return (
        <div>
            ...
            <main className="app">
                ...
                {activeCategories.movies && <Movies items={dataToShow.movies.filter((m) => m.title.toLowerCase().includes(query.toLowerCase()))} />}
                {activeCategories.tvShows && dataToShow.tvShows.length > 0 && <TVShows items={dataToShow.tvShows.filter((s) => s.title.toLowerCase().includes(query.toLowerCase()))} />}
                {activeCategories.specials && dataToShow.specials.length > 0 && <Specials items={dataToShow.specials.filter((s) => s.title.toLowerCase().includes(query.toLowerCase()))} />}
                {activeCategories.originals && dataToShow.originals.length > 0 && <Originals items={dataToShow.originals.filter((o) => o.title.toLowerCase().includes(query.toLowerCase()))} />}
            </main>
        </div>
    );
}
